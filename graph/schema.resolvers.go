...
func (r *mutationResolver) AddReview(ctx context.Context, input model.ReviewInput) (*model.Review, error) {
	...
	newReview := &store.Review{
		...
		SellerID:  uint32(input.SellerID),
		Rating:    input.Rating,
		ProductID: uint32(input.ProductID),
		CreatedAt: time.Now().UTC(),
		UpdatedAt: time.Now().UTC(),
	}
	err := storeHandler.AddReview(ctx, newReview)
	if err != nil {
		return nil, err
	}
...
func (r *queryResolver) Reviews(ctx context.Context, id string, value string) ([]*model.Review, error) {
	...
	if id == "storeID" || id == "orderID" {

		result, err := storeHandler.GetReviews(ctx, id, uint32(v))
		if err != nil {
			return nil, err
		}
		for _, val := range result {
			orderID, _ := strconv.Atoi(val.OrderID)
			myReviews := &model.Review{
				StoreID:   int(val.StoreID),
				ProductID: int(val.ProductID),
				OrderID:   orderID,
				SellerID:  int(val.SellerID),
				Rating:    val.Rating,
				Buyer: &model.ReviewBuyer{
					Nickname: val.Buyer.Nickname,
					Comment:  val.Buyer.Comment,
					Avatar:   val.Buyer.Avatar,
				},
				CreatedAt: &val.CreatedAt,
				UpdatedAt: &val.UpdatedAt,
			}
			output = append(output, myReviews)
		}
		return output, nil
	}
	result, err := storeHandler.GetReviews(ctx, id, uint32(v))
